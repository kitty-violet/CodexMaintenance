...
这个项目不是只适用于某一台电脑。程序不会硬编码用户名、盘符或 `X:\Tools` 这类本机路径。

默认规则是：

- `CodexMaintenance.config` 优先放在程序所在目录。
- `BackupRoot=backups` 表示备份放在配置文件旁边的 `backups` 文件夹。
- `.codex` 目录默认建议使用 `%USERPROFILE%\.codex`。
- 如果用户想把备份放到 D 盘、E 盘或其它路径，可以在图形界面里选择，或用 `--configure` 修改。

也就是说，别人下载后可以直接解压到任意目录，例如：
...
然后双击 `CodexMaintenanceGui.exe` 使用。

注意：不同电脑仍需要满足基本运行条件，例如 Windows、.NET Framework、以及可用的 `sqlite3.exe`。如果没有 SQLite，工具会先备份，但会跳过日志数据库清理。
## 推荐：使用图形界面
